...
scp -q "$OUT/nodeguard_kern.o" "$OUT/nodeguard-maps.spec" \
    ...
    "$REPO"/etc/tmpfiles-nodeguard.conf \
...
if [ -d /etc/suricata ]; then
    ...
    echo "WARNING: /etc/suricata missing (suricata RPM not installed yet); yaml staged only"
    install -m 0644 "$S/suricata.yaml" /usr/local/lib/nodeguard/suricata.yaml.staged
fi
...
for u in nodeguard-maps nodeguard-xdp nodeguard-responder nodeguard-sweep \
         nodeguard-watchdog suricata-update; do
    systemd-analyze verify "/etc/systemd/system/$u.service" 2>&1 | grep -v 'Unit is bound' || true
done
