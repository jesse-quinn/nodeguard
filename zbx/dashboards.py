...
def build_security(ctx):
    ...
    # Attack-origin view: the live world map (regenerated by nodeguard-geo
    # every 5 minutes, served at --attack-map-url) beside the top attacker
    # countries. The map widget is omitted when no URL is configured.
    if ctx.attack_map_url:
        widgets.append(lib.url_widget(
            0, y, 48, 10, "Blocked attack sources (live, dot size = hits)",
            ctx.attack_map_url))
    geo = [(48, "top attacker countries", "geo_summary")]
    gy = y
    for h in ctx.hosts:
        iid = ctx.itemid(h, kv("geo_summary"))
        if iid is not None:
            widgets.append(lib.itemvalue(
                48, gy, 24, 5, "%s: top attacker countries" % h["name"],
                iid))
        gy += 5
    return widgets
